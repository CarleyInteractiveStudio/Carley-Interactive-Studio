...
const CLIENT_CONFIGS = {
    ...
    'carleyinteractivestudio.github.io': {
        name: 'VidSpri',
        logo: 'carley_foto_web/Logo VidSpri.png',
        msg: 'Inicia sesión para convertir tus videos en sprites con VidSpri.'
    },
    'localhost': {
        name: 'Local Dev Site',
        logo: 'carley_foto_web/Logo_C.png',
...
function handleSSOSuccess(session) {
    ...
    try {
        ...
        let isWhitelisted = WHITELISTED_DOMAINS.some(domain =>
            redirectUrl.hostname === domain || redirectUrl.hostname.endsWith('.' + domain)
        );

        // Strict path check for GitHub Pages domain to avoid unauthorized apps
        if (redirectUrl.hostname === 'carleyinteractivestudio.github.io') {
            if (!redirectUrl.pathname.startsWith('/VidSpri')) {
                isWhitelisted = false;
            }
        }
